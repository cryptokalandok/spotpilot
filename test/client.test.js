...
test('getPrice calls the public ticker endpoint and extracts last price', async () => {
  ...
  assert.equal(calls[0].url, 'https://safe.trade/api/v2/trade/public/tickers/prlusdt');
...
test('getBalances authenticates, normalizes and filters balances', async () => {
  const calls = [];
  const client = createClient(calls, () => jsonResponse([
    { currency: 'prl', balance: '10.5', locked: '1.25' },
    { currency: 'usdt', balance: '22', locked: '0' },
    { currency: 'btc', balance: '0.1', locked: '0' },
  ]));

  const result = await client.getBalances({ coins: 'PRL,USDT' });
  ...
  assert.deepEqual(
    ...
    [
      ...
      { asset: 'USDT', total: '22', available: '22', locked: '0' },
    ],
  );
});
